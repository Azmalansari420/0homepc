	...
	public function booknow()
{
    ...
    $existingUser = $this->crud->selectDataByMultipleWhere('registration', [
        'email' => $email,
        'mobile' => $mobile
    ]);
    ...
    if (empty($existingUser)) {
        $registrationdata = [
            "role" => 1,
            "username" => $name,
            "password" => $mobile, // You should hash the password
            "email" => $email,
            "mobile" => $mobile,
            "address" => $address,
            "status" => $status,
        ...
        $this->crud->insert('registration', $registrationdata);
    }

    // Insert booking data
    $this->crud->insert('booknow', $bookdata);
