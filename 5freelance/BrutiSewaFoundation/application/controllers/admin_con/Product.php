   ...
   //--check user login or not
	...
	{		
		...
		if(isset($_POST['submit']))
		{
			...
			if (!empty($_FILES['multiple_image_json']['name'][0])) 
			{
			    ...
			    foreach ($filename as $key => $value) 
			    {
			        if (!empty($filename[$key])) 
			        {
			            ...
			            $unique_name = time() . '_' . $key . '.jpg';
			            $upload_path = $this->arr_values['upload_path'];
			            $final_path  = $upload_path . $unique_name;
			            compress_image($tempname[$key], $final_path, 75);
			...
			$this->db->delete("product_image",array("p_id"=>$insert_id,));
	        if(!empty($allimage))
	        {
...
//---------------------status
	...
	public function get_subcategory()
    {
        ...
        $city = $this->db
            ->get_where('sub_categories', array(
                'cat_id' => $cat_id,
                'status' => 1
            ))
            ->result_object();
