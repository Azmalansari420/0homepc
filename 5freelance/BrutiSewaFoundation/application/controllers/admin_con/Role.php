      ...
      function UpdateData()
		{
			...
			if(isset($_POST['submit']))
			{
				...
				$access = [];
				$access_count = $this->input->post('access_count'); // menu keys
				$main_access  = $this->input->post('main_access');  // checked main menu

				foreach ($access_count as $key)
				{
				    $inner = $this->input->post('inner_access'.$key);

				    if (!empty($inner)) {
				        $access[$key] = $inner;   // save checked inner access
				    } else {
				        $access[$key] = [];
				    }
				}
				...
				else
				{
					...
					$getrole_access = $this->db->get_where('tbl_admin', array('role' => $insert_id))->result_object();

					$this->db->update("tbl_admin", array('access' => $role_access), array('id' => $getrole_access[0]->id));
				}
