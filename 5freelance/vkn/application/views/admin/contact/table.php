  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="btn-col text-nowrap" width="1%">
        <?php
        if($data->mark_as_read==1)
          { ?>
        <button type="button" class="btn btn-xs btn-success mark-read" data-id="<?=$data->id ?>">Mark as Read</button>
      <?php } ?>
     ...
     $('.mark-read').on('click', function() {
        ...
        Swal.fire({
           ...
           if (result.isConfirmed) 
           {
              $.ajax({
                 type: 'POST',
                 url: '<?=base_url("admin_con/contact/mark_as_read")?>',
                 data: {id: id},
                 success: function(response) {
                 console.log(response);
                 location.reload();
