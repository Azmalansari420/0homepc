...
// ----------forget password---------
	...
	function dashboard()
	{
		...
		$data['total_projects'] = $this->db->count_all('survey_projects');

		$data['total_complete'] = $this->db
		    ->where('status',1)
		    ->count_all_results('survey_responses');

		$data['total_terminate'] = $this->db
		    ->where('status',2)
		    ->count_all_results('survey_responses');

		$data['total_quota'] = $this->db
		    ->where('status',3)
		    ->count_all_results('survey_responses');

		$data['total_security'] = $this->db
		    ->where('status',4)
		    ->count_all_results('survey_responses');
