...
function generate_project_code($CI)
    {
        $CI->db->select('project_code');
        $CI->db->order_by('id', 'DESC');
        $CI->db->limit(1);

        $query = $CI->db->get('survey_projects');

        if ($query->num_rows() > 0)
        {
            $lastCode = $query->row()->project_code;

            $number = (int) str_replace('AI', '', $lastCode);

            $number++;

            return 'AI' . str_pad($number, 4, '0', STR_PAD_LEFT);
        }
        else
        {
            return 'AI0001';
...
function compress_image($source, $destination, $quality = 75)
{
 ...
 // compress_image_png($temp_path, $final_path, 6);
function compress_image_png($source, $destination, $compression = 6)
{
    $info = @getimagesize($source);
    ...
    // PNG save (0 = no compression, 9 = max compression)
  ...
  function generate_order_id()
    {
        ...
        if ($last && !empty($last->order_id) && preg_match('/ORD_(\d{4})-(\d+)/', $last->order_id, $matches)) {
        ...
        return 'ORD_' . $year . '-' . str_pad($new_number, 3, '0', STR_PAD_LEFT); // 001 format
    }
...
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('survey_projects',array('id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->project_name;
  }
  else
  {
    $name = "Not Found..";
